...
def optimize_ratings(df):
    ...
    if rating_stats['std'] < 2.0:
        ...
        df['Book-Rating'] = df['Book-Rating'].apply(lambda x: max(1, min(10, x * 2)))
    return df
...
def train_model(trainset, user_based=True):
    if user_based:
        sim_options = {
            ...
            'user_based': True,
            'shrinkage': 100,
            'min_support': 2  # azaltıldı
        }
        print(f"\n🔧 User-Based Model Eğitiliyor (KNN)...")
        model = KNNBasic(k=30, min_k=3, sim_options=sim_options, verbose=True)
    else:
        print(f"\n🔧 Item-Based Model Eğitiliyor (SVD)...")
